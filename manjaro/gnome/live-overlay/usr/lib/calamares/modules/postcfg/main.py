...
class ConfigController:
    ...
    def run(self):
        ...
        # Install Office Suite or minimal set if selected (WIP)
        package = libcalamares.globalstorage.value("packagechooser_packagechooser")
        if package == 'minimal_install':
        ...
        print('package removal completed')
            #libcalamares.utils.warning("no office suite selected, {!s}".format(package))
        if not package:
                libcalamares.utils.warning("no office suite selected, {!s}".format(package))
        else:
 
            # For PoC we added the Office Packages to mhwd-live overlay in 18.1.0
            cmd = ["pacman", "-S", package, "--noconfirm", "--config", "/opt/mhwd/pacman-mhwd.conf" ]
            self.mkdir("opt/mhwd")
            self.mount("opt/mhwd")
